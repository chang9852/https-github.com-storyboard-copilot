  ...
  for (let index = 0; index < points.length - 1; index += 1) {
    ...
    for (const rect of rects) {
      if (isVertical) {
        ...
        if (horizontalIntersectsRect(from.y, from.x, to.x, rect)) return true;
      }
    }
  }
  return false;
...
function pickLaneY(
  ...
  const lowerBound = rects.length > 0 ? Math.max(...rects.map((rect) => rect.bottom)) + LANE_GAP : targetY + 80;
