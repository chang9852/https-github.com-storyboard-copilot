...
export function buildGenerationErrorReport(
  ...
  if (context.providerId) sections.push(`- Provider: ${context.providerId}`);
  if (context.requestModel) sections.push(`- Model: ${context.requestModel}`);
  if (context.requestSize) sections.push(`- Size: ${context.requestSize}`);
  if (context.requestAspectRatio) sections.push(`- Aspect Ratio: ${context.requestAspectRatio}`);
