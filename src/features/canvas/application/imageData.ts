...
function createImagePipelineError(message: string, details?: string, cause?: unknown): ErrorWithDetails {
  ...
  if (cause !== undefined) detailParts.push(`cause: ${stringifyUnknown(cause)}`);
...
export async function loadImageElement(source: string): Promise<HTMLImageElement> {
  ...
  return await new Promise((resolve, reject) => {
    ...
    image.onerror = () =>
      reject(createImagePipelineError('Image load failed', `source=${source}`));
...
export async function imageUrlToDataUrl(imageUrl: string): Promise<string> {
  ...
  if (isLikelyLocalImagePath(imageUrl)) {
    if (isTauri()) {
      try {
        ...
        throw createImagePipelineError('Cannot read local image', `source=${imageUrl}`, error);
      }
    }
    const localResponse = await fetch(resolveImageDisplayUrl(imageUrl));
    if (!localResponse.ok) {
      throw createImagePipelineError('Cannot read local image', `source=${imageUrl}`);
    }
  ...
  if (!response.ok) {
    throw createImagePipelineError('Cannot download image', `url=${imageUrl}`);
  }
...
export async function prepareNodeImageFromFile(
  ...
  if (canUseLocalPath) {
    return await prepareNodeImage(normalizedPath, maxPreviewDimension);
  }
  ...
  return await prepareNodeImage(source, maxPreviewDimension);
}
...
function renderPreviewDataUrl(
  ...
  if (longestSide <= maxDimension) return sourceDataUrl;
  ...
  if (mimeType === 'image/jpeg') return canvas.toDataURL(mimeType, 0.86);
...
export async function prepareNodeImage(
  ...
  if (!trimmedImageUrl) {
    throw createImagePipelineError('No image available', 'imageUrl is empty');
  }
  ...
  try {
    ...
    throw createImagePipelineError('Cannot parse image', `source=${trimmedImageUrl}`, error);
  }
}
