    ...
    }),
    {
      ...
      migrate: (persistedState: unknown) => {
        const state = (persistedState ?? {}) as {
          apiKey?: string;
          apiKeys?: ProviderApiKeys;
          providerConfigs?: Record<string, { apiKey: string; enabled: boolean }>;
          ignoreAtTagWhenCopyingAndGenerating?: boolean;
          grsaiNanoBananaProModel?: string;
          hideProviderGuidePopover?: boolean;
        ...
        // Legacy migration: if apiKeys is empty but providerConfigs has keys, migrate
        const legacyApiKeys: Record<string, string> = {};
        if (state.providerConfigs) {
          for (const [id, config] of Object.entries(state.providerConfigs)) {
            if (config.apiKey) {
              legacyApiKeys[id] = config.apiKey;
            }
          }
        }
